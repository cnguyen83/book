...
code; in Rust, one of those tools is *generics*. Generics are stand-ins that
represent multiple concrete types or scopes. We can express properties of
...
parameters. Similarly, we'll talk about using generics in struct and enum
definitions too.
...
situations and have the compiler check that references will never go out of
scope before the value they're referring to does.

## Removing Duplication by Extracting a Function

Before gettng into generics syntax, let's first review a technique for dealing
with duplication that doesn't use generics: extracting a function. Once that's
fresh in our minds, we'll use the same mechanics with generics to extract a
generic function! In the same way that you recognize duplicated code to extract
...
The function takes an argument, `numbers`, which represents any slice of `i32`
values that we might pass into the function. The code in the function
definition operates on the `numbers` representation of any `&[i32]`. When we
